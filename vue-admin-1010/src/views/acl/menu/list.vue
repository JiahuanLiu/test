<template>
  <div class="app-container">
    ...
    <el-table :data="menuList" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" style="width: 100%;margin-bottom: 20px;" row-key="id" border default-expand-all>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-button v-if="(scope.row.level == 1 || scope.row.level == 2) && hasPerm('permission.add')" id="button" type="text" size="mini" @click="() => {dialogFormVisible = true, menu.pid = scope.row.id}">添加菜单
          </el-button>
          <el-button v-if="scope.row.level == 3 && hasPerm('permission.add')" id="button" type="text" size="mini" @click="() => {dialogPermissionVisible = true, permission.pid = scope.row.id}">添加功能
          </el-button>
          <el-button v-if="scope.row.level == 4 && hasPerm('permission.update')" id="button" type="text" size="mini" @click="() => updateFunction(scope.row)">修改功能
          </el-button>
          <el-button v-if="scope.row.level != 4 && hasPerm('permission.update')" id="button" type="text" size="mini" @click="() => getById(scope.row)">修改
          </el-button>
          <el-button v-if="hasPerm('permission.remove')" id="button" type="text" size="mini" @click="() => remove(scope.row)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="dialogFormValue">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button id="button" @click="restData()">取 消</el-button>
        <el-button id="button" type="primary" @click="append()">确 定</el-button>
      </div>
    </el-dialog>
    <!-- 添加功能的窗口 -->
    <el-dialog :visible.sync="dialogPermissionVisible" title="添加功能">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button id="button" @click="restData()">取 消</el-button>
        <el-button id="button" type="primary" @click="appendPermission()">确 定</el-button>
      </div>
    </el-dialog>
