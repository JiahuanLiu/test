  ...
  <div>
    <div class="mt20 c-infor-box" style="margin-left: 150px; margin-right: 150px">
      <div class="i-box">
        ...
        <!-- 分页 -->
        <!-- <div class="tac">
              <el-pagination v-if="total > limit" :current-page="page" :page-size="limit" :total="total" background layout="total, prev, pager, next, jumper" @current-change="gotoCoursePage" />
            </div> -->
        <!-- 课程列表结束-->
        <!-- </div>
        </article> -->
        ...
        <!-- 分页 -->
        <!-- <div class="tac">
              <el-pagination v-if="total > limit" :current-page="page" :page-size="limit" :total="total" background layout="total, prev, pager, next, jumper" @current-change="gotoQuestionPage" />
            </div>
          </div>
        </div> -->
        <!-- 参与的讨论，结束 -->
            ...
            <el-tab-pane label="基本资料修改">
              <el-form ref="userInfo" :model="userInfo" label-width="100px" style="margin-left: 50px">
                <el-form-item label="头像">
                  ...
                  <!-- <el-upload :show-file-list="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload" :action="BASE_API + '/eduoss/fileoss'" class="avatar-uploader"> -->
                  <el-upload :show-file-list="true" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload" :action="baseURL + '/eduoss/fileoss'" class="avatar-uploader">
                    <img v-if="userInfo.avatar" :src="userInfo.avatar" class="avatar">
                    <i v-else class="el-icon-plus avatar-uploader-icon" />
                  </el-upload>
                </el-form-item>
                <el-form-item label="用户名">
                  <el-input :disabled="false" id="input" v-model="userInfo.nickname" />
                </el-form-item>
                ...
                <el-form-item label="个人简介">
                  <el-input v-model="userInfo.sign" type="textarea" id="input" />
                </el-form-item>
                <el-form-item>
                  <el-button type="primary" id="button" @click="updateUserInfo">保存</el-button>
                </el-form-item>
              </el-form>
            </el-tab-pane>
            <el-tab-pane label="密码修改">
              <el-form ref="passwordVo" :model="passwordVo" label-width="100px" style="margin-left: 50px">
                <el-form-item :rules="[{ required: true, message: '请输入原密码', trigger: 'blur' }]" prop="oldPassword" label="原密码">
                  <el-input v-model="passwordVo.oldPassword" type="password" placeholder="请输入原密码" id="input" />
                </el-form-item>
                <el-form-item :rules="[{ required: true, message: '请输入新密码', trigger: 'blur' }]" prop="newPassword" label="新密码">
                  <el-input v-model="passwordVo.newPassword" type="password" placeholder="设置新密码" id="input" />
                </el-form-item>
                <el-form-item>
                  <el-button type="primary" id="button" @click="updatePassword">提交修改</el-button>
                </el-form-item>
              </el-form>
            </el-tab-pane>
          </el-tabs>
...
import staffCourse from '@/api/staffCourse'
import staffVideo from '@/api/staffVideo'
import staff from '@/api/staff'
import login from '@/api/login'
import question from '@/api/login'
import answer from '@/api/login'
import cookie from 'js-cookie'
...
export default {
  ...
  methods: {
    ...
    updatePassword () {
      ...
      this.passwordVo.newPassword = encrypt(this.passwordVo.newPassword)
      this.passwordVo.oldPassword = encrypt(this.passwordVo.oldPassword)
